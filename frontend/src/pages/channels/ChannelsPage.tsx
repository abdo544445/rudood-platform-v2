...
export const ChannelsPage: React.FC = () => {
  ...
  const handlePollTelegram = async () => {
    ...
    soundEngine.playClick();
    try {
      const res = await apiClient.post('/channels/telegram/poll');
      if (res.data.success) {
        soundEngine.playSuccess();
        toast.success(res.data.message || 'تم فحص ومزامنة تيليجرام بنجاح ✓');
        fetchChannels();
      }
    } catch (e: any) {
      toast.error(e.response?.data?.message || 'تعذر فحص رسائل تيليجرام');
    } finally {
        ...
        <SpotlightCard className="p-6 bg-slate-900/80 border border-white/5 space-y-4" spotlightColor="rgba(36, 129, 204, 0.14)">
          <div className="flex items-center justify-between border-b border-white/5 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-sm font-bold text-white flex items-center gap-2">
                  <span>بوت تليجرام (Telegram Bot)</span>
                  {channels.telegram?.is_connected && (
                ...
                <p className="text-[11px] text-slate-400">ربط مباشر وسريع عبر توكن BotFather مع دعم الاستماع الحي</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={isPollingTelegram || !tgForm.bot_token}
                title="فحص الرسائل الجديدة فوراً"
                className="px-3 py-1.5 rounded-xl text-xs font-bold bg-sky-500/15 hover:bg-sky-500/25 border border-sky-500/30 text-sky-300 transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-40"
              >
                <RefreshCw className={`w-3.5 h-3.5 ${isPollingTelegram ? 'animate-spin text-sky-400' : ''}`} />
                <span>{isPollingTelegram ? 'جاري الفحص...' : 'فحص ومزامنة الرسائل ⚡'}</span>
              </button>
          ...
          {channels.telegram?.last_error && (
            <div className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/20 text-xs text-rose-300 flex items-center gap-2">
              ...
              <span>تنبيه الاتصال: {channels.telegram.last_error}</span>
            </div>
          )}
          ...
          >
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {tgForm.bot_username && (
                  <a
                    href={`https://t.me/${tgForm.bot_username.replace('@', '')}`}
                    target="_blank"
                    rel="noreferrer"
                    className="px-3 py-2.5 rounded-xl bg-sky-500/20 hover:bg-sky-500/30 border border-sky-500/40 text-sky-300 text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1"
                  >
                    <span>فتح البوت ↗</span>
                  </a>
                )}
            ...
            <div className="flex justify-end pt-1">
              <button type="submit" disabled={isSaving} className="px-5 py-2.5 rounded-xl gold-btn text-xs font-bold flex items-center gap-1.5 cursor-pointer">
                <Save className="w-3.5 h-3.5" />
                <span>حفظ وربط إعدادات تليجرام ✓</span>
              </button>
            </div>
          </form>
        </SpotlightCard>
